...
本项目面向轻量级视频处理场景，适合本地快速剪辑、素材整理、功能演示和教学实验。它优先保证部署简单、处理路径清晰和用户数据边界明确，而不是替代专业非线性剪辑软件。

应用的核心处理链路运行在浏览器端：用户选择本地视频后，浏览器将文件交给 FFmpeg.wasm 执行裁剪、转码、滤镜、音频调整等操作。Node.js 服务不参与视频计算，只负责提供静态资源和必要的跨域隔离响应头。

登录后的处理历史和素材库用于保存结果文件与任务元数据，方便回看、下载和继续编辑。这些能力是持久化增强，不改变“原始输入文件默认留在本地浏览器”的设计边界。
