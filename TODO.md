...
本文件维护项目的待办任务，供团队成员查阅与认领。难度标注 S（1 天内）/ M（2–5 天）/ L（1 周以上）；「入门」列标 ✓ 的任务适合新成员上手。认领前请阅读对应任务的「前置」依赖。
...
| 编号 | 任务 | 难度 | 入门 | 前置 | 入口 / 说明 |
|---|---|---|---|---|---|
| TD-1 | 引入构建 / 模块体系（打包器或原生 ESM），替代顺序 `<script>` 加载 | L | | — | `src/index.html` 脚本列表、模块全局命名空间；多数大型功能的前置 |
...
| 编号 | 任务 | 难度 | 入门 | 前置 | 入口 / 说明 |
|---|---|---|---|---|---|
| C3 | 处理前后对比（原视频与结果并排） | S | ✓ | — | `src/modules/app.js` showResult、结果区 DOM |
| C4 | 键盘快捷键（空格播放、`[` `]` 标记起止） | S | ✓ | — | `src/modules/preview.js` |
| C5 | 高分辨率 / 长时长上传预警 | S | ✓ | — | 读取视频元数据，`upload.js` / `preview.js` |
| C1 | 失败任务一键重试 | S | ✓ | — | `src/modules/history.js`，复用原参数重跑 |
...
| 编号 | 任务 | 难度 | 入门 | 前置 | 入口 / 说明 |
|---|---|---|---|---|---|
| A1 | 项目管理（新建 / 切换 / 重命名 / 删除） | M | | — | `src/services/projectService.js` + 项目选择 UI + `app.js` |
| A3 | 账户管理（修改密码 / 注销 / 个人资料） | M | | — | Supabase Auth API；注销需级联清理 Storage |
| A2 | 项目协作（成员邀请 / 角色分配） | L | | A1、待决策 1 | invitations 表已就绪 |
| A4 | 操作审计展示 | M | | A1 | audit_events 表已建，需补充写入 |

### E. 工程化与移动端

| 编号 | 任务 | 难度 | 入门 | 前置 | 入口 / 说明 |
|---|---|---|---|---|---|
| E1 | APK 原生保存到相册 / 系统分享 | M | | — | `@capacitor/filesystem`、`@capacitor/share`；`app.js` download |
| D2 | PWA 离线（Service Worker 缓存应用与 wasm） | M | | — | manifest 已具备 |
| D3 | 错误上报 / 可观测性 | M | | — | 引入上报服务或自建 |
| D4 | 国际化（i18n） | M | | — | 文案当前为硬编码中文 |
| D5 | 无障碍（ARIA / 焦点管理 / 对比度） | M | | — | 全量审查 |
| E2 | 原生 FFmpeg 提速 | L | | 待决策 3 | ffmpeg-kit 已停止维护，需评估替代方案 |
| E3 | Release 签名与上架准备 | M | | 待决策 3 | keystore / 隐私政策 / App Bundle |
