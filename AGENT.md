...
- If `npx.cmd supabase login`, `npx.cmd supabase link --project-ref wwqgixluxlegrttyhrgy`, `npx.cmd supabase db push --dry-run`, or any remote Supabase command fails with authentication, authorization, or project access errors, confirm the collaborator is using their own Supabase account and has accepted the project invite. If they are not a project member, ask the project administrator or owner to invite them into Supabase project `wwqgixluxlegrttyhrgy`; do not try to solve access failures by sharing publishable keys or `service_role` keys.
- If a collaborator cannot log in, cannot link the project, or cannot run remote Supabase commands, they may still prepare database changes locally by writing timestamped SQL files under `supabase/migrations/`. After they commit or submit those files, a project administrator or owner with Supabase access must pull the code, review the SQL, run `npx.cmd supabase db push --dry-run`, and only then run `npx.cmd supabase db push`. Collaborators without project access must not edit the Dashboard manually and must not ask for shared `service_role` keys.
